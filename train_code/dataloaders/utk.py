...
class UTKTrain(Dataset):
    def __init__(self, cfg, tau=5, dataset_dir='dataset/UTK/'):
        ...
        self.std = torch.as_tensor(imagenet_stats['std'], dtype=torch.float32)
...
class UTKTest(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        std = torch.as_tensor(imagenet_stats['std'], dtype=dtype, device=test_image_trans.device)
...
class UTKRef(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        ref_age = self.df_base['age'].iloc[idx]
        ref_image = Image.open(ref_name)
        ref_image_trans = ImgAugTransform_Test_Aug(ref_image).astype(np.float32) / 255.
        ...
        std = torch.as_tensor(imagenet_stats['std'], dtype=dtype, device=ref_image_trans.device)
        ref_image_trans.sub_(mean[:, None, None]).div_(std[:, None, None])

        sample = dict()
        sample['ref_impath'] = ref_name
        sample['ref_age'] = ref_age
...
class UTKRefSampling(Dataset):
    ...
    def ref_sampling(self, pair_num=5):
        ...
        for base_min_age in base_age_unique:
            ...
            base_max_idx = np.where(base_age == base_max_age)[0]
